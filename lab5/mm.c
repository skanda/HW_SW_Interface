...
void* mm_malloc (size_t size) {
  ...
  if (size <= MIN_BLOCK_SIZE) {
    ...
    // Round up for correct alignmentl
...
/*	if((blockSize-reqSize)>= MIN_BLOCK_SIZE)
	{
		newSize = blockSize - reqSize;
		precedingBlockUseTag = ptrFreeBlock->sizeAndTags & TAG_PRECEDING_USED;
		ptrFreeBlock->sizeAndTags = reqSize | precedingBlockUseTag | TAG_USED;
		removeFreeBlock(ptrFreeBlock);
		BlockInfo *ptrFollowingBlock = (BlockInfo *)POINTER_ADD(ptrFreeBlock, SIZE(ptrFreeBlock->sizeAndTags));
		ptrFollowingBlock->sizeAndTags = newSize | TAG_PRECEDING_USED;
		size_t *ptrFBlockBTag = (size_t *)POINTER_ADD(ptrFollowingBlock,(SIZE(ptrFollowingBlock->sizeAndTags)-WORD_SIZE));
		*ptrFBlockBTag = newSize | TAG_PRECEDING_USED;
		insertFreeBlock(ptrFollowingBlock);
	}
	else
	{
		tempBlock = (BlockInfo *)POINTER_ADD(ptrFreeBlock,SIZE(ptrFreeBlock->sizeAndTags));
	//	tempSize = SIZE(tempBlock->sizeAndTags);
		tempBlock->sizeAndTags  |= TAG_PRECEDING_USED;
		precedingBlockUseTag = ptrFreeBlock->sizeAndTags & TAG_PRECEDING_USED;
	...
	if((blockSize-reqSize)>= MIN_BLOCK_SIZE)
		{
			newSize = blockSize - reqSize;
			precedingBlockUseTag = ptrFreeBlock->sizeAndTags & TAG_PRECEDING_USED;
			BlockInfo *ptrFollowingBlock = (BlockInfo *)POINTER_ADD(ptrFreeBlock, reqSize);
			ptrFollowingBlock->sizeAndTags = newSize | TAG_PRECEDING_USED;
			size_t *ptrFBlockBTag = (size_t *)POINTER_ADD(ptrFollowingBlock,(SIZE(ptrFollowingBlock->sizeAndTags)-WORD_SIZE));
			*ptrFBlockBTag = newSize | TAG_PRECEDING_USED;
			insertFreeBlock(ptrFollowingBlock);
		...
		{
			tempBlock = (BlockInfo *)POINTER_ADD(ptrFreeBlock,blockSize);
			tempSize = SIZE(tempBlock->sizeAndTags);
			tempBlock->sizeAndTags |= TAG_PRECEDING_USED;
		}
		removeFreeBlock(ptrFreeBlock);
...
void mm_free (void *ptr) {
 ...
 //payloadSize = SIZE(blockInfo->sizeAndTags);
  blockInfo->sizeAndTags = SIZE(blockInfo->sizeAndTags) | precedingBlockUseTag;
  followingBlock = POINTER_ADD((BlockInfo *)blockInfo,SIZE(blockInfo->sizeAndTags));
  followingBlock->sizeAndTags &=~TAG_PRECEDING_USED;
  
  size_t *BlockFBTag = (size_t *)POINTER_ADD(blockInfo,SIZE(blockInfo->sizeAndTags)-WORD_SIZE);
  
  *BlockFBTag = (blockInfo->sizeAndTags);
 // examine_heap();
